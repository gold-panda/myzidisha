...
if (substr_count($picture, 'file:') > 0 && file_exists( str_replace('file:', DOCUMENT_DIR, $picture)))
{
	$img = imagecreatefromjpeg(ltrim(rtrim(str_replace('file:', DOCUMENT_DIR, $picture))));
	
}

if(!$img){
	$img = imagecreatefromjpeg(ltrim(rtrim(str_replace('file:', DOCUMENT_DIR, DEFAULT_IMAGE))));
}
