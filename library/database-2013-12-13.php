...
//added by Julia 21-10-2013
	function find_lenders($sort='FirstName', $dir='asc', $start=0, $limit='', $country, $search)
	{
		...
		if($country=='AA'){
			...
			$q="SELECT lenders.userid, regdate, last_login, lenders.FirstName, lenders.LastName, lenders.City, lenders.Country, lenders.Email, lenders.Active, concat(REPLACE(`FirstName`,' ',''), REPLACE(`LastName`,' ','')) as sortname, users.username  FROM ! join users on users.userid = lenders.userid where users.userid NOT IN(SELECT userid from ! where emailVerified=?) AND (users.username like '%$search%' OR lenders.FirstName like '%$search%' OR lenders.LastName like '%$search%' OR lenders.City like '%$search%' OR lenders.Country like '%$search%' OR lenders.Email like '%$search%') order by ".$sort." ".$dir." limit ".$start.", ".$limit;
			$result=$db->getAll($q,array('lenders', 'users', 0));	
			}
		}else{
			if(empty($search)){
			...
			$q="SELECT lenders.userid, regdate, last_login, lenders.FirstName, lenders.LastName, lenders.City, lenders.Country, lenders.Email, lenders.Active, concat(REPLACE(`FirstName`,' ',''), REPLACE(`LastName`,' ','')) as sortname, users.username  FROM ! join users on users.userid = lenders.userid where users.userid NOT IN(SELECT userid from ! where emailVerified=?) AND (lenders.Country like '$country') AND (users.username like '%$search%' OR lenders.FirstName like '%$search%' OR lenders.LastName like '%$search%' OR lenders.City like '%$search%' OR lenders.Country like '%$search%' OR lenders.Email like '%$search%') order by ".$sort." ".$dir." limit ".$start.", ".$limit;
			$result=$db->getAll($q,array('lenders', 'users', 0));	
			}
		}
		return $result;
		...
		if($country=='AA'){
			if($brwr_type=='all'){
				if(empty($search)){
					...
					$q="SELECT b.userid, b.FirstName, b.LastName, b.PAddress, b.City, b.Country, b.TelMobile, b.Email,b.Active, b.lastVisited, b.admin_notes, concat(REPLACE(b.`FirstName`,' ',''), REPLACE(b.`LastName`,' ','')) as sortname FROM ! as b where b.FirstName like '%$search%' || b.LastName like '%$search%' || b.TelMobile like '%$search%' || b.Email like '%$search%' group by b.userid";
					$result=$db->getAll($q,array('borrowers'));
				}
			}
			elseif($brwr_type=='endorser'){
				if(empty($search)){
					...
					$q="SELECT b.userid, b.FirstName, b.LastName, b.PAddress, b.City, b.Country, b.TelMobile, b.Email,b.Active, b.lastVisited, b.admin_notes, concat(REPLACE(b.`FirstName`,' ',''), REPLACE(b.`LastName`,' ','')) as sortname FROM ! as b where b.endorser=? AND (b.FirstName like '%$search%' || b.LastName like '%$search%' || b.TelMobile like '%$search%' || b.Email like '%$search%') group by b.userid";
					$result=$db->getAll($q,array('borrowers', 1));
				}
			}
			elseif($brwr_type=='pndng_sub'){
				if(empty($search)){
					...
					$q="SELECT b.userid, b.FirstName, b.LastName, b.PAddress, b.City, b.Country, b.TelMobile, b.Email,b.Active, b.lastVisited, b.admin_notes, u.emailVerified, b.iscomplete_later, u.facebook_id, b.Created, concat(REPLACE(b.`FirstName`,' ',''), REPLACE(b.`LastName`,' ','')) as sortname FROM ! as b join ! as u on b.userid=u.userid where b.endorser<>? AND (b.FirstName like '%$search%' || b.LastName like '%$search%' || b.TelMobile like '%$search%' || b.Email like '%$search%') group by b.userid";
					$result1=$db->getAll($q,array('borrowers', 'users', 1));
					foreach($result1 as $row){
						$endorser_count= $this->IsEndorsedComplete($row['userid']);
						if(($row['Created']>'1370995200' && $row['facebook_id']!='' && $endorser_count<$this->getAdminSetting('MinEndorser')) || $row['iscomplete_later']==1 || $row['emailVerified']==0){
			...
			elseif($brwr_type=='pndng_act'){
				if(empty($search)){
					...
					$q="SELECT b.userid, b.FirstName, b.LastName, b.PAddress, b.City, b.Country, b.TelMobile, b.Email,b.Active, b.lastVisited, b.admin_notes, b.Created, concat(REPLACE(b.`FirstName`,' ',''), REPLACE(b.`LastName`,' ','')) as sortname FROM ! as b join ! as u on b.userid=u.userid where b.iscomplete_later=? AND b.Active<>? AND b.Assigned_status<>? AND u.emailVerified=? AND (b.FirstName like '%$search%' || b.LastName like '%$search%' || b.TelMobile like '%$search%' || b.Email like '%$search%') group by b.userid";
					$result1=$db->getAll($q,array('borrowers', 'users', 0, 1, 2, 1));
					foreach($result1 as $row){
						if($row['Created']>'1370995200'){ 
							$user_fb= $this->IsFacebook_connect($row['userid']);
			...
			elseif($brwr_type=='decline'){
				if(empty($search)){
					...
					$q="SELECT b.userid, b.FirstName, b.LastName, b.PAddress, b.City, b.Country, b.TelMobile, b.Email,b.Active, b.lastVisited, b.admin_notes, concat(REPLACE(b.`FirstName`,' ',''), REPLACE(b.`LastName`,' ','')) as sortname FROM ! as b where b.Assigned_status=? AND (b.FirstName like '%$search%' || b.LastName like '%$search%' || b.TelMobile like '%$search%' || b.Email like '%$search%') group by b.userid";
					$result=$db->getAll($q,array('borrowers', 2));
			...
			elseif($brwr_type=='active'){
				if(empty($search)){
					...
					$q="SELECT b.userid, b.FirstName, b.LastName, b.PAddress, b.City, b.Country, b.TelMobile, b.Email,b.Active, b.lastVisited, b.admin_notes, concat(REPLACE(b.`FirstName`,' ',''), REPLACE(b.`LastName`,' ','')) as sortname FROM ! as b where b.Active=? AND (b.FirstName like '%$search%' || b.LastName like '%$search%' || b.TelMobile like '%$search%' || b.Email like '%$search%') group by b.userid";
					$result=$db->getAll($q,array('borrowers', 1));
			...
			if($brwr_type=='all'){
				if(empty($search)){
					...
					$q="SELECT b.userid, b.FirstName, b.LastName, b.PAddress, b.City, b.Country, b.TelMobile, b.Email,b.Active, b.lastVisited, b.admin_notes, concat(REPLACE(b.`FirstName`,' ',''), REPLACE(b.`LastName`,' ','')) as sortname FROM ! as b where b.Country=? AND b.FirstName like '%$search%' || b.LastName like '%$search%' || b.TelMobile like '%$search%' || b.Email like '%$search%' group by b.userid";
					$result=$db->getAll($q,array('borrowers', $country));
			...
			elseif($brwr_type=='endorser'){
				if(empty($search)){
					...
					$q="SELECT b.userid, b.FirstName, b.LastName, b.PAddress, b.City, b.Country, b.TelMobile, b.Email,b.Active, b.lastVisited, b.admin_notes, concat(REPLACE(b.`FirstName`,' ',''), REPLACE(b.`LastName`,' ','')) as sortname FROM ! as b where b.endorser=? AND b.Country=? AND (b.FirstName like '%$search%' || b.LastName like '%$search%' || b.TelMobile like '%$search%' || b.Email like '%$search%') group by b.userid";
					$result=$db->getAll($q,array('borrowers', 1, $country));
			...
			elseif($brwr_type=='pndng_sub'){
				if(empty($search)){
					...
					$q="SELECT b.userid, b.FirstName, b.LastName, b.PAddress, b.City, b.Country, b.TelMobile, b.Email,b.Active, b.lastVisited, b.admin_notes, u.emailVerified, b.iscomplete_later, u.facebook_id, b.Created, concat(REPLACE(b.`FirstName`,' ',''), REPLACE(b.`LastName`,' ','')) as sortname FROM ! as b join ! as u on b.userid=u.userid where b.endorser<>? AND b.Country=? AND (b.FirstName like '%$search%' || b.LastName like '%$search%' || b.TelMobile like '%$search%' || b.Email like '%$search%') group by b.userid";
					$result1=$db->getAll($q,array('borrowers', 'users', 1, $country));
					foreach($result1 as $row){
						$endorser_count= $this->IsEndorsedComplete($row['userid']);
						if(($row['Created']>'1370995200' && $row['facebook_id']!='' && $endorser_count<$this->getAdminSetting('MinEndorser')) || $row['iscomplete_later']==1 || $row['emailVerified']==0){
			...
			elseif($brwr_type=='pndng_act'){
				if(empty($search)){
					...
					$q="SELECT b.userid, b.FirstName, b.LastName, b.PAddress, b.City, b.Country, b.TelMobile, b.Email,b.Active, b.lastVisited, b.admin_notes, b.Created, concat(REPLACE(b.`FirstName`,' ',''), REPLACE(b.`LastName`,' ','')) as sortname FROM ! as b join ! as u on b.userid=u.userid where b.iscomplete_later=? AND b.Active<>? AND b.Assigned_status<>? AND u.emailVerified=? AND b.Country=? AND (b.FirstName like '%$search%' || b.LastName like '%$search%' || b.TelMobile like '%$search%' || b.Email like '%$search%') group by b.userid";
					$result1=$db->getAll($q,array('borrowers', 'users', 0, 1, 2, 1, $country));
					foreach($result1 as $row){
						if($row['Created']>'1370995200'){ 
							$user_fb= $this->IsFacebook_connect($row['userid']);
			...
			elseif($brwr_type=='decline'){
				if(empty($search)){
					...
					$q="SELECT b.userid, b.FirstName, b.LastName, b.PAddress, b.City, b.Country, b.TelMobile, b.Email,b.Active, b.lastVisited, b.admin_notes, concat(REPLACE(b.`FirstName`,' ',''), REPLACE(b.`LastName`,' ','')) as sortname FROM ! as b where b.Assigned_status=? AND b.Country=? AND (b.FirstName like '%$search%' || b.LastName like '%$search%' || b.TelMobile like '%$search%' || b.Email like '%$search%') group by b.userid";
					$result=$db->getAll($q,array('borrowers', 2, $country));
			...
			elseif($brwr_type=='active'){
				if(empty($search)){
					...
					$q="SELECT b.userid, b.FirstName, b.LastName, b.PAddress, b.City, b.Country, b.TelMobile, b.Email,b.Active, b.lastVisited, b.admin_notes, concat(REPLACE(b.`FirstName`,' ',''), REPLACE(b.`LastName`,' ','')) as sortname FROM ! as b where b.Active=? AND b.Country=? AND (b.FirstName like '%$search%' || b.LastName like '%$search%' || b.TelMobile like '%$search%' || b.Email like '%$search%') group by b.userid";
					$result=$db->getAll($q,array('borrowers', 1, $country));
...
function updateBorrower($uname, $namea, $nameb, $pass1, $post, $city, $country, $email, $mobile,$reffered_by, $income, $about, $bizdesc, $id, $bnationid,  $language, $community_name_no, $repaidPast, $debtFree,$share_update,$onbehalf, $behalf_name, $behalf_number, $behalf_email, $behalf_town,$borrower_behalf_id, $completeLater, $bfamilycont1,$bfamilycont2,$bfamilycont3, $bneighcont1, $bneighcont2, $bneighcont3, $home_no, $rec_form_offcr_name, $rec_form_offcr_num, $refer_member,  $volunteer_mentor, $fb_data, $endorser_name, $endorser_email, $endorser_id)
	...
	function updateLoanApp($userid, $loanid, $amount, $interest, $loanuse, $inst_day, $gperiod)
	{
		global $db;
		...
		$damount=$amount/$rate;
		$r = "UPDATE ! set Amount =?, interest =?, loanuse =?, reqdamt =?,  installment_day=?, grace =? where loanid = ? AND borrowerid=?";
		$res=$db->query($r, array('loanapplic', $amount, $interest,$loanuse, $damount,$inst_day, $gperiod, $loanid, $userid));
		return $res;
	}
...
//added by Julia 6-11-2013
	...
	{	
		...
		if(!empty($res1))
		{
			...
			for($i=0; $i < $cards; $i++)
			{
				$card_code = getCardCode12($date,$i+1);
