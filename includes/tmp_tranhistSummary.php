		...
		<table class="detail">
			<tbody>
				<tr>
					...
					<td>
						<input type="hidden" name="tmp_transactionhistorySummary" id="transactionhistorySummary">
			...
			foreach($set1 as $row1) {
				//logger('set1 in foreach');
					...
					if( $userLevel == BORROWER_LEVEL){
						$tranamt=round(($row1['amount'] /($row1['conversionrate']!=0?$row1['conversionrate']:1)),4);
					} else {
						$tranamt=$row1['amount'];
					}
					...
					if($row1['txn_type'] == FUND_UPLOAD) {
						...
						$int=$value * $finalrate;
						$fee = $value*$webfee;
							$feetxn = $database->tmp_getprevfeetxn($row1['id']);
