...
<?php
	...
	if(!empty($_POST) && $session->userlevel == ADMIN_LEVEL) {
		...
		$q="SELECT email,users.userid  FROM `borrowers` join users on `borrowers`.userid= users.userid WHERE `Active` = 1 $where ORDER BY userid DESC limit $limit";
		$content='';
...
?>
<div align='left' class='static'><h1>Borrowers Email list:</h1></div>

	<form method="post" action="">
		Borrower Registered from:<br/>
		<input type="text" name="datefrom" id='datefrom'><br/><br/>
